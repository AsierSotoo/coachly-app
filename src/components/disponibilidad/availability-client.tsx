...
export function AvailabilityClient({
        ...
        <div className="px-4 py-4 border-b flex items-center gap-3"
          ...
          {teamLogo
            ...
            : <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black"
                style={{ backgroundColor: '#1e293b', color: '#4be277' }}>{teamName.charAt(0)}</div>
          }
          <div>
            <p className="text-xs font-semibold" style={{ color: '#adb4ce' }}>{teamName}</p>
            <p className="text-[10px]" style={{ color: '#475569' }}>{isMatch ? 'Partido' : 'Entrenamiento'} · {eventDate}</p>
          </div>
        </div>
          ...
          {players.map(p => (
            <button key={p.id} onClick={() => selectPlayer(p.id)}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold" style={{ color: '#dce1fb' }}>{p.name}</p>
              ...
              <span className="material-symbols-outlined flex-shrink-0" style={{ fontSize: 18, color: '#2e3447' }}>
                chevron_right
              </span>
            </button>
          ))}
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#080d1e', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div className="sticky top-0 z-10 border-b" style={{ backgroundColor: '#0c1324', borderColor: '#1e293b' }}>
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold truncate" style={{ color: '#adb4ce' }}>{teamName}</p>
          </div>
        ...
        {/* Jugador activo */}
        {selectedPlayer && (
          <div className="flex items-center gap-2 px-4 py-2 border-t" style={{ borderColor: '#1e293b', backgroundColor: 'rgba(167,139,250,0.05)' }}>
      ...
      <div className="px-4 pt-5 pb-3">
        {isMatch ? (
          <div className="flex items-center justify-center gap-5">
            <div className="flex flex-col items-center gap-1">
              ...
              <p className="text-[10px] font-semibold text-center max-w-[70px] leading-tight" style={{ color: '#adb4ce' }}>{teamName}</p>
            </div>
            <span className="text-xs font-black" style={{ color: '#475569' }}>VS</span>
            <div className="flex flex-col items-center gap-1">
              {rivalLogo
              ...
              <p className="text-[10px] font-semibold text-center max-w-[70px] leading-tight" style={{ color: '#adb4ce' }}>
        ...
        <p className="text-center text-sm font-semibold capitalize mt-3" style={{ color: '#dce1fb' }}>{eventDate}</p>
        {eventSubtitle && (
          <p className="text-center text-xs mt-0.5" style={{ color: '#64748b' }}>{eventSubtitle}</p>
        )}
      </div>
      ...
      {selectedId && (
        <div className="px-3 mb-4">
          <p className="text-[10px] font-bold uppercase tracking-widest mb-2 px-1" style={{ color: '#475569' }}>
            Tu confirmación
          </p>
          <div className="rounded-2xl border-2 overflow-hidden"
            ...
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <p className="text-sm font-bold flex-1" style={{ color: '#dce1fb' }}>{selectedPlayer?.name}</p>
                ...
                return (
                  <form key={key} action={async (fd: FormData) => {
                    ...
                    <button type="submit" disabled={isSubmitting}
                      ...
                      style={{ backgroundColor: active ? s.bg : 'transparent', color: active ? s.color : '#64748b' }}>
            ...
            {players.filter(p => p.id !== selectedId && avail.has(p.id)).map(p => {
              ...
              return (
                <div key={p.id} className="flex items-center gap-3 px-4 py-2.5 rounded-xl"
                  ...
                  <p className="text-sm flex-1 truncate" style={{ color: '#adb4ce' }}>{p.name}</p>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0"
          ...
          <div className="flex flex-wrap gap-2 px-1">
            {players.filter(p => p.id !== selectedId && !avail.has(p.id)).map(p => (
              <span key={p.id} className="text-[11px] px-2.5 py-1 rounded-full"
                style={{ backgroundColor: '#0f172a', color: '#475569', border: '1px solid #1e293b' }}>
                {p.name.split(' ')[0]}
              </span>
            ))}
          </div>
