...
export function AvailabilityCoachCard({ players, responses }: Props) {
  ...
  return (
    <section className="rounded-2xl border overflow-hidden" style={{ borderColor: '#2a342d', backgroundColor: '#111713' }}>
      {/* Cabecera */}
      <div className="flex items-center justify-between px-5 py-3 border-b" style={{ borderColor: '#2a342d' }}>
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined" style={{ fontSize: 18, color: '#89968e' }}>how_to_reg</span>
          <h2 className="text-xs font-bold uppercase tracking-wider" style={{ color: '#89968e' }}>Disponibilidad</h2>
        </div>
        <div className="flex items-center gap-2 text-[11px] font-bold">
          ...
          {none.length > 0 && total > 0 && (
            <span className="px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(173,180,206,0.08)', color: '#637168' }}>{none.length} sin responder</span>
          )}
          {total === 0 && (
            <span className="text-[11px]" style={{ color: '#637168' }}>Sin respuestas aún</span>
          )}
        </div>
      </div>
      ...
      {total === 0 ? (
        <div className="flex flex-col items-center gap-1.5 py-7 px-4 text-center">
          ...
          <p className="text-xs" style={{ color: '#637168' }}>Nadie ha respondido todavía al enlace</p>
        </div>
      ) : (
