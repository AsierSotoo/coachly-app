...
export function AvailabilityPicker({ matchId, seasonId, players, initialAvailability }: Props) {
  ...
  function toggle(playerId: string, status: 'available' | 'doubt' | 'unavailable') {
    const current = avail[playerId]
    const next: Status = current === status ? null : status
    setAvail(prev => ({ ...prev, [playerId]: next }))
    startTransition(() => {
      setPlayerAvailability(matchId, playerId, next, seasonId)
    })
  }
          ...
          return (
            <div key={player.id} className="flex items-center gap-3 px-4 py-2.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {player.position && <p className="text-[10px] text-slate-500">{player.position}</p>}
              </div>
              {/* Botones de estado */}
