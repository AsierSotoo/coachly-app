...
export function SortableStatsTable({ stats, playerLabel, teamId }: { stats: PlayerStat[]; playerLabel: string; teamId?: string }) {
  ...
  function ColHeader({ label, col, title }: { label: string; col: SortKey; title?: string }) {
    const active = sortKey === col
    return (
      <th className="px-3 py-3 text-[10px] font-bold uppercase tracking-widest text-center cursor-pointer select-none hover:opacity-80 transition-opacity"
        style={{ color: active ? '#4be277' : '#adb4ce' }}
      ...
      <table className="w-full text-left" style={{ minWidth: 640 }}>
        <thead>
          <tr className="border-b border-[#1e293b]" style={{ backgroundColor: '#151b2d' }}>
            <th className="px-3 py-3 text-[10px] font-bold uppercase tracking-widest text-center" style={{ color: '#adb4ce' }}>#</th>
            <th className="px-3 py-3 text-[10px] font-bold uppercase tracking-widest text-left" style={{ color: '#adb4ce' }}>{playerLabel}</th>
            <th className="px-3 py-3 text-[10px] font-bold uppercase tracking-widest text-center" style={{ color: '#adb4ce' }}>Pos</th>
            <ColHeader label="PJ"   col="gamesPlayed" title="Partidos jugados" />
            <ColHeader label="Min'" col="minutes"     title="Minutos totales" />
          ...
          {sorted.map((s, i) => {
            ...
            return (
              <tr key={s.playerId} className="border-b border-[#1e293b] last:border-0"
                ...
                <td className="px-3 py-2.5">
                  {teamId ? (
                    <Link href={`/dashboard/team/${teamId}/players/${s.playerId}`} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                      ...
                      <span className="text-xs font-semibold text-white truncate hover:underline" style={{ maxWidth: 130 }}>{s.name}</span>
                    </Link>
                  ) : (
                ...
                <td className="px-3 py-2.5 text-center">
                  {s.yellowCards > 0
                    ? <span className="text-xs font-bold" style={{ color: s.yellowCards >= YELLOW_WARNING ? '#facc15' : '#adb4ce' }}>{s.yellowCards}</span>
